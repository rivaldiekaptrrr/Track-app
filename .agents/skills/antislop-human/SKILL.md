...
- **Tell:** `outline: none` or `outline: 0` with no replacement focus styles.
...
- **Fix:** every interactive element is reachable and operable by keyboard (R-32): logical tab order following visual order, activation with Enter or Space, and dialogs closeable with Escape (R-26).
...
- [ ] Is every interactive element reachable and operable by keyboard, with dialogs closeable via Escape and no `outline: none` without a replacement? (R-32, R-26)
